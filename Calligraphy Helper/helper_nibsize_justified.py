...
def manuscript_format(doc, margins, nib_width, versal_height, spacing, justified):
    ...
    while w < len(width_list):
        ...
        while i < len(words):
            ...
            if nib_width == 2:
                temp_df = temp_df.drop(columns=['nib15'])
                temp_df.rename(columns = {'nib2':'values'},inplace=True)
            if nib_width == 1.5:
                temp_df = temp_df.drop(columns=['nib2'])
                temp_df.rename(columns = {'nib15':'values'},inplace=True)
            temp_df['cumsum'] = temp_df['values'].cumsum()
            values = temp_df['values'].tolist()
            word_length = word_length + (sum(values)+ (1* (values.count(1) // 2)))
            if i < (len(words)-1):
                word_length = word_length+2
            if line_length+word_length <= line_width+1:
               line.append(word)    
               line_length = line_length+word_length
            else:
                if justified == True and len(word) >=6 and line_width - line_length >= 8:
                    # determine how much room there is left on the line
                    room_left = line_width - line_length
                    first_half = temp_df[temp_df['cumsum']<room_left]
                    first_half = first_half.append({'letter':'-','values':2,'cumsum':0},ignore_index=True)
                    first_half_word = ''.join([str(word) for word in (first_half['letter'])])
                    first_half_length = sum(first_half['values'])
                    second_half = temp_df[temp_df['cumsum']>=room_left]
                    second_half_word = ''.join([str(word) for word in (second_half['letter'])])
                    second_half_length = sum(second_half['values'])
                    if len(second_half_word)>=3:
                        line.append(first_half_word)
                        line_length += first_half_length
                        
                        word = second_half_word
                        word_length = second_half_length
...
manuscript_format(kahless, [4.5,4], 1.5, 2, 0, True)
